...
from models.transformer_fixpe_region import Transformer, TransformerEncoder, TransformerDecoderLayer, \
...
def getdelta(best_cider,model, dataloader, text_field):
    import itertools
    model.eval()
    gen = {}
    gts = {}
    ...
    large=np.sum(np.array(_['CIDEr'])>best_cider)
    small=np.sum(np.array(_['CIDEr'])<best_cider)
    delta=small/large
    return delta
...
def train_xe(model, dataloader, optim, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, captions) in enumerate(dataloader):
            detections=ia.randnChooseOne5(detections)
            detections, captions = detections.to(device), captions.to(device)
            out = model(detections, captions)
            optim.zero_grad()
...
def train_scst(model, dataloader, optim, cider, text_field):
    ...
    with tqdm(desc='Epoch %d - train' % e, unit='it', total=len(dataloader)) as pbar:
        for it, (detections, caps_gt) in enumerate(dataloader):
            ...
            reward=((best_cider**(delta-1))*reward)**(1.0/(delta))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default='Trans_region_EMFM')
    ...
    parser.add_argument('--features_path', type=str,default='/home/sdb1/jijiayi/Data/coco_detections.hdf5')
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if not use_rl:
            ...
            delta=getdelta(best_cider,model, dict_dataloader_val, text_field)
            print('delta:',delta)
        ...
        if switch_to_rl:
            copyfile('saved_models/%s_best.pth' % args.exp_name, 'saved_models/%s_ex_best.pth' % args.exp_name)
